...
def generate_trajectory(traj_type, duration, dt):
    ...
    if traj_type == 'line':
        ...
        for i in range(4):
            ...
            psi_segment = np.full(segment_steps, np.arctan2(y_end - y_start, x_end - x_start)-np.pi ) 
            x.extend(x_segment)
            y.extend(y_segment)
...
def main():
    ...
    try:
        while not rospy.is_shutdown() and trajectory_index < len(trajectory):
            ...
            control_inputs.append(u_body.copy())
            msg = Twist()
        ...
        save_to_mat("backwards_circle.mat", t_array, state_xi_array, pos_ref_array, control_inputs_array)
