...
def generate_trajectory(traj_type, duration, dt):
    ...
    if traj_type == 'line':
        ...
        x_center, y_center, radius = 0.0, 0.0, 0.5
...
def main():
    ...
    # MPC parameters
    Npred, dt, n, m = 7, 0.1, 6, 6

    xmin, xmax = np.array([-np.inf, -np.inf, 0.1, -np.pi/8, -np.pi/8, -np.inf]), \
                 np.array([np.inf, np.inf, 0.4, np.pi/8, np.pi/8, np.inf])  

    umin = np.array([-0.5, -0.5, -0.5, -0.5, -0.5, -0.5])
    umax = np.array([ 0.5,  0.5,  0.5,  0.5,  0.5,  0.5])
    

    Q = np.diag([40, 40, 5, 1, 1, 30])  
    ...
    traj_type = 'square'
    trajectory = generate_trajectory(traj_type, 20, dt)
    ...
    try:
        while not rospy.is_shutdown():
            ...
            # new_umin, new_umax = feedback_linearizer.treat_constraints(roll, pitch, yaw, umin, umax)
            u_des = linear_MPC(Npred, x0, u0, n, m, dt, Q, R,xmin, xmax, umin, umax, xref, roll, pitch, yaw)
