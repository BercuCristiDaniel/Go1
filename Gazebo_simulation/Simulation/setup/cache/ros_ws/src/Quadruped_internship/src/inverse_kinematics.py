...
class Go1LegDynamics:
    ...
    def compute_tau_ff(self, q, dq, xdd_des, Fe):
        # Compute dynamics terms
        M = self.mass_matrix(q)
        C = self.coriolis_matrix(q, dq)
        g = self.gravity_vector(q)
        J = self.jacobian(q)[0:3, :]  # Only linear part for 3D foot task space
        Jdot = self.jacobian_dot(q, dq)[0:3, :]  # Time derivative of linear part

        Mx_inv = np.linalg.pinv(J @ np.linalg.pinv(M) @ J.T)

        # Compute tau_ff
        tau_ff = (
            M @ np.linalg.pinv(J) @ (xdd_des - Jdot @ dq - np.linalg.inv(Mx_inv) @ Fe)
            + C @ dq
            + g
            + J.T @ Fe
        )
