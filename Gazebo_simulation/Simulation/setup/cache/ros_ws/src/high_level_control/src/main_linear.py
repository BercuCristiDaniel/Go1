...
def main():
    ...
    try:
        while not rospy.is_shutdown() and trajectory_index < len(trajectory):
            ...
            msg = Twist()
            msg.linear.x = u_des[0]
            msg.linear.y = u_des[1]
            msg.angular.z = u_des[2]
            pub.publish(msg)
            u0 = u_des
        ...
        save_to_mat("linear_mpc_control_input.mat", t_array, state_xi_array, pos_ref_array, control_inputs_array)
