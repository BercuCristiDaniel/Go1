...
class QuadrupedVelocityTransformer:
    def __init__(self, L, C):
        self.L = L  # Half of the robot length
        self.C = C  # Half of the robot width
    ...
    def transform_body_to_leg_velocities(self, vx, vy, omega_z):
        ...
        v_fd = np.sqrt(v_f**2 + v_d**2)  # Front right
        v_fe = np.sqrt(v_f**2 + v_e**2)  # Front left
        v_td = np.sqrt(v_t**2 + v_d**2)  # Rear right
        v_te = np.sqrt(v_t**2 + v_e**2)  # Rear left

        theta_fd = np.arctan2(v_f, v_d)  # Front right
        theta_fe = np.arctan2(v_f, v_e)  # Front left
        theta_td = np.arctan2(v_t, v_d)  # Rear right
        theta_te = np.arctan2(v_t, v_e)  # Rear left
