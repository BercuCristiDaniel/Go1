...
#         }
#         num_points = int(1.0 * control_rate)
...
#         stride_length, _ = calc_cp_freqP(v_leg)
...
#         swing_mask = u_eval < 0.5
#         trajectory = np.zeros((num_points, 3))
#         velocity = np.zeros((num_points, 3))
#         acceleration = np.zeros((num_points, 3))

#         bez_traj = evaluate_bezier(u_eval, W)
#         bez_vel = evaluate_bezier_velocity(u_eval, W)
#         bez_acc = evaluate_bezier_acceleration(u_eval, W)

#         for i, u in enumerate(u_eval):
#             if swing_mask[i]:
#                 trajectory[i] = bez_traj[i]
#                 velocity[i] = bez_vel[i]
#                 acceleration[i] = bez_acc[i]
#             else:
#                 # Linear stance interpolation
#                 start = control_pts[3]
#                 end = control_pts[0]
#                 alpha = (u - 0.5) / 0.5
#                 trajectory[i] = start + alpha * (end - start)
#                 velocity[i] = (end - start) / T_p
#                 acceleration[i] = np.zeros(3)

#         trajectories[leg_name] = trajectory
#         bez_velocities[leg_name] = velocity
#         accelerations[leg_name] = acceleration

#     return trajectories, bez_velocities, accelerations


def generate_all_leg_trajectories(vx, vy, omega_z, walking_type, control_rate=250): 
    C = 0.47  
    L = 0.30  
    # if vy != 0:
    ...
    if abs(vx) < epsilon and abs(vy) < epsilon and abs(omega_z) < epsilon:
        static_positions = {
            "front_left":  np.array([ 0.225,  0.15, -0.30]),
            "front_right": np.array([ 0.225, -0.15, -0.30]),
            "rear_left":   np.array([-0.225,  0.15, -0.35]),
            "rear_right":  np.array([-0.225, -0.15, -0.35])
        }
    ...
    initial_foot_positions = {
        "front_left": np.array([ 0.225,  0.15, -0.30]),
        "front_right": np.array([ 0.225, -0.15, -0.30]),
        "rear_left": np.array([-0.225,  0.15, -0.32]),
        "rear_right": np.array([-0.225, -0.15, -0.32])
    }
...
def generate_rise_trajectories(z_start=-0.05, z_end=-0.30, duration=5.0, control_rate=250):
    """
    ...
    initial_foot_positions = {
        "front_left":  np.array([ 0.225,  0.15, z_start]),
        "front_right": np.array([ 0.225, -0.15, z_start]),
        "rear_left":   np.array([-0.225,  0.15, z_start]),
        "rear_right":  np.array([-0.225, -0.15, z_start]),
    }
...
if __name__ == "__main__":
    ...
    trajectories, vels, accs = generate_all_leg_trajectories(vx, vy, omega_z, "trot")
    plot_single_leg_trajectory_scatter(vels)